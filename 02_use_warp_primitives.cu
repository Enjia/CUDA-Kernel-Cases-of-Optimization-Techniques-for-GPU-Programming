...
// 优化点：利用warp-level原语在warp内高效交换数据，减少共享内存与同步
__global__ void reduce_warp(const float* in, float* out, int n){
  ...
  for(int offset=16; offset>0; offset>>=1)
    sum += SHFL_DOWN(0xffffffff, sum, offset);
...
int main(){
  ...
  float *d_in,*d_out1,*d_out2; int blocks = (n+512-1)/512;
  CHECK(cudaMalloc(&d_in,bytes));
  CHECK(cudaMalloc(&d_out1,blocks*sizeof(float)));
  CHECK(cudaMalloc(&d_out2,blocks*sizeof(float)));
  CHECK(cudaMemcpy(d_in,h,bytes,cudaMemcpyHostToDevice));
  CHECK(cudaMemset(d_out2,0,blocks*sizeof(float)));

  dim3 block(256), grid(blocks);
  cudaEvent_t s,e; CHECK(cudaEventCreate(&s)); CHECK(cudaEventCreate(&e));

  CHECK(cudaEventRecord(s));
  reduce_shared<<<grid,block>>>(d_in,d_out1,n);
  CHECK(cudaEventRecord(e)); CHECK(cudaEventSynchronize(e));
  float t1; CHECK(cudaEventElapsedTime(&t1,s,e));

  CHECK(cudaEventRecord(s));
  reduce_warp<<<grid,block>>>(d_in,d_out2,n);
  CHECK(cudaEventRecord(e)); CHECK(cudaEventSynchronize(e));
  float t2; CHECK(cudaEventElapsedTime(&t2,s,e));

  float *h1=(float*)malloc(blocks*sizeof(float));
  float *h2=(float*)malloc(blocks*sizeof(float));
  CHECK(cudaMemcpy(h1,d_out1,blocks*sizeof(float),cudaMemcpyDeviceToHost));
  CHECK(cudaMemcpy(h2,d_out2,blocks*sizeof(float),cudaMemcpyDeviceToHost));
  printf("max diff: %g\n", max_diff(h1,h2,blocks));
  printf("shared reduce: %.3f ms, warp reduce: %.3f ms\n", t1,t2);

  free(h); free(h1); free(h2);
  CHECK(cudaFree(d_in)); CHECK(cudaFree(d_out1)); CHECK(cudaFree(d_out2));
  return 0;
}
